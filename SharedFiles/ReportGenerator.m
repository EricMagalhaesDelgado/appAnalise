function [htmlReport, peaksTable] = ReportGenerator(Data, reportInfo, exceptionList)
    ...
    specDataTypes = class.Constants.specDataTypes;
    ...
    for ii = 1:numel(Template)
        ...
        while kk < NN
            ...
            if Template(ii).Recurrence && ~ismember(specData(jj).MetaData.DataType, specDataTypes)
                continue
            else
                kk = kk+1;
            end
    ...
    Footnote1 = sprintf('<b>appAnálise</b> v. %s, <b>fiscaliza</b> v. %s, <b>anateldb</b> %s', appVersion.appAnalise.Version, appVersion.fiscaliza, appVersion.anateldb.ReleaseDate);
...
function value = Fcn_Source(Children, reportInfo, idx)
    ...
    switch Source
        ...
        case 'Node';         value = specData(idx).Node;
        case 'ThreadID';     value = specData(idx).ThreadID;
        ...
        case 'Samples';      value = specData(idx).Samples;
        ...
        case 'minLevel';     value = sprintf('%.1f %s', min(specData(idx).statsData(:,1)), specData(idx).MetaData.metaString{1});
        case 'maxLevel';     value = sprintf('%.1f %s', max(specData(idx).statsData(:,4)), specData(idx).MetaData.metaString{1});
        case 'FileFormat';   value = specData(idx).FileFormat;
        case 'TaskName';     value = specData(idx).TaskName;
        case 'Description';  value = specData(idx).Description;
        case 'RelatedFiles'; value = strjoin(specData(idx).RelatedFiles, ', ');
        ...
        case 'Parameters'
            if isempty(specData(idx).MetaData.metaString{4})
                Operation = specData(idx).MetaData.metaString{3};
            else
                Operation = strjoin(specData(idx).MetaData.metaString(3:4), '-');
            end

            Resolution = '';
            if ~isempty(specData(idx).MetaData.metaString{2})
                Resolution = sprintf('Resolução: %s', specData(idx).MetaData.metaString{2});
            end
            ...
            value = sprintf('GPS: %.6f, %.6f (%s); Operação: %s; Unidade: %s; %s', specData(idx).gps.Latitude,           ...
                                                                                   ...
                                                                                   Operation,                            ...
                                                                                   specData(idx).MetaData.metaString{1}, ...
                                                                                   Resolution);
    end
            ...
            switch Source
                ...
                case 'specData'
                    ...
                    for jj = 1:numel(specData)
                        if ismember(specData(jj).MetaData.DataType, specDataTypes)
                            ll = ll+1;
