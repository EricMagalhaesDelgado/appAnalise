function GeneralTable = Channel(specData, idxThread)
    ...
    chTable  = struct2table(specData.UserData.channelManual);
    ...
    for ii = 1:height(chTable)
        ...
        chStart    = chTable.Band(ii, 1);
        chStop     = chTable.Band(ii, 2);
        ...
        for jj = 1:numel(idxEmissions)
            idxEmission = idxEmissions(jj);
    
            EmissionFrequency    = specData.UserData.Emissions.Frequency(idxEmission);
            idxEmissionFrequency = freq2idx(chStart, chStop, height(chMatrix), EmissionFrequency*1e+6);
    
            EmissionDescription  = specData.UserData.Emissions.UserData(idxEmission).Description;
            if isnumeric(EmissionDescription)
                EmissionDescription = '';
            end

            EmissionTable(jj, :) = {jj,                                                               ...
                                    EmissionFrequency,                                                ...
                                    1000 * specData.UserData.Emissions.BW(idxEmission),               ...
                                    100 * sum(occMatrix(idxEmissionFrequency, :)) / width(occMatrix), ...
                                    EmissionDescription};
        end
    
        GeneralTable(ii, :) = {extractBefore(chTable.Name{ii}, ' @'), ...
...
function Template = TableTemplate(Type, Height)
    switch Type
        case 'GeneralTable'
            Template = table('Size',          [Height, 10],                                                                                  ...
                             'VariableNames', {'Transponder', 'Frequência central (MHz)', 'Largura (kHz)', 'Referência', 'FBO estimada (%)', ...
                                               'FBO mínima (%)', 'FBO média (%)', 'FBO máxima (%)', 'FCO (%)', 'Emissões'},                  ...
                             'VariableTypes', {'cell', 'double', 'double', 'cell', 'double', 'double', 'double', 'double', 'double', 'cell'});
        case 'EmissionTable'
