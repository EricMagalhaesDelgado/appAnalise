function specData = FileReader_RFlookBinV2(filename, ReadType, metaData)
    ...
    % !! BUG !!
    % Problema na decodificação de caracteres especiais que eventualmente
    % podem existir no cabeçalho textual do RFlookBin v.2/1 - em especial
    % os campos "Description" (ç, á, ã etc) e "LevelUnit" (µ).
...
function specData = Fcn_MetaDataReader(rawData, filename)
    ...
    MetaStruct     = jsondecode(char(rawData(23:22+nMetaStruct)));
